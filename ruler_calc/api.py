...
class ApiClient:
    ...
    def main2(self, debug=False):
        '''
        メイン処理
        '''
        rotate_path = self.tmp_dir + "/rotate.jpg"
        ApiClient.rotate(self.input_path, rotate_path)

        # otsu_path = self.tmp_dir + "/otsu.jpg"
        # ApiClient.otsu_s(rotate_path, otsu_path)
        
        ad_path = self.tmp_dir + "/ad.jpg"
        ApiClient.ad_s(rotate_path, ad_path)

        otsu_path = ad_path
        
        saisen_path = self.tmp_dir + "/skelton.jpg"
        ApiClient.skelton_s(otsu_path, saisen_path)

        lsd_path = self.tmp_dir + "/lsd.jpg"
        ApiClient.hlsd_s(saisen_path, saisen_path, lsd_path)

        '''
        sums = ApiClient.get_up_down(saisen_path)
        x_values, y_values = ApiClient.show_freq(sums, xlabel="画像のy軸の値", ylabel="1と0の変化の数")

        if debug:
            ApiClient.show_xy(x_values, y_values, xlabel="画像のy軸の値", ylabel="1と0の変化の数")
        ...
        x_index, x_values, y_values = ApiClient.get_line(saisen_path)

        line_path = self.tmp_dir + "/line.jpg"
        ApiClient.write_line(saisen_path, line_path, x_index)

        row_path = self.tmp_dir + "/row.jpg"
        ApiClient.save_row_img(saisen_path, row_path, x_index)

        

        freq, ws = ApiClient.get_ws(row_path)

        # ここも

        x_values, y_values = ApiClient.show_freq(freq, xlabel="点の間隔", ylabel="頻度")

        pixelPerMM = ApiClient.get_pixel_per_mm(ws) #  + 2

        # ラスト

        result_path = self.tmp_dir + "/result.jpg"
        ApiClient.write_lines(rotate_path, result_path, pixelPerMM)
    ...
    @staticmethod
    def get_line(path):
        sums = ApiClient.get_up_down(path)
        x_values, y_values = ApiClient.show_freq(sums)
    ...
    def get_mode_value(ws):
        ...
        mode_values = ApiClient.get_mode_values(ws)
        avg = np.mean(ws)
    ...
    def get_pixel_per_mm(ws):
        ...
        return ApiClient.get_mode_value(ws)
    ...
    @staticmethod
    def main(path, task_id, tmp_dir = "tmp", isHorizontal = True, debug=False):
        ins = ApiClient(path, tmp_dir = f"{tmp_dir}/{task_id}", isHorizontal = isHorizontal)
        ins.main2(debug=debug)
    ...
    def otsu(self):
        ...
        ApiClient.otsu_s(input_path, output_path)
    ...
    def skelton(self):
        ...
        ApiClient.skelton_s(input_path, output_path)
